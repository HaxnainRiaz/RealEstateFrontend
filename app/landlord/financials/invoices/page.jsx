    ...
    return (
        <div className="space-y-[40px]">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-[24px]">
                <div>
                    <span className="text-[12px] font-[700] text-[#1D4ED8] uppercase tracking-[0.1em] mb-[8px] block">Billing Protocol</span>
                    <h1 className="text-[32px] font-[700] text-[#111827]">Invoice Management</h1>
                    <p className="text-[16px] text-[#6B7280]">Automated billing cycles and resident payment tracking.</p>
                </div>
                ...
                {[
                    ...
                    <motion.div key={stat.label} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
                        <Card className="bg-[#FFFFFF] border-[#D1D5DB]/30 p-[32px]">
                            ...
                            <h3 className={`text-[24px] font-[700] ${stat.color}`}>{stat.val}</h3>
                        </Card>
                    </motion.div>
                ))}
            </div>
            ...
            <div className="bg-[#FFFFFF] rounded-[16px] border border-[#D1D5DB]/30 overflow-hidden shadow-sm">
                <div className="p-[24px] border-b border-[#D1D5DB]/20 bg-[#F9FAFB]/50 flex justify-between items-center">
                    <h3 className="text-[18px] font-[700] text-[#111827]">Invoice Ledger</h3>
                    <div className="flex gap-[12px]">
                        <button className="px-[16px] py-[8px] bg-[#FFFFFF] border border-[#D1D5DB] rounded-[6px] text-[12px] font-[700] text-[#6B7280]">Filter by Status</button>
                        <button className="px-[16px] py-[8px] bg-[#FFFFFF] border border-[#D1D5DB] rounded-[6px] text-[12px] font-[700] text-[#6B7280]">Export CSV</button>
                    </div>
                </div>
                <table className="w-full text-left">
                    <thead>
                        <tr className="bg-[#F9FAFB] border-b border-[#D1D5DB]/50">
                            <th className="p-[20px] text-[11px] font-[700] uppercase text-[#6B7280]">Invoice ID</th>
                            ...
                            <th className="p-[20px] text-[11px] font-[700] uppercase text-[#6B7280]">Due Date</th>
                    ...
                    <tbody className="divide-y divide-[#D1D5DB]/20">
                        {INVOICES.map((invoice) => (
                            <tr key={invoice.id} className="hover:bg-[#F9FAFB]/50 transition-colors">
                                <td className="p-[20px]">
                                    <p className="text-[14px] font-[700] text-[#111827]">{invoice.id}</p>
                                    <p className="text-[11px] text-[#6B7280]">{invoice.date}</p>
                                </td>
                                <td className="p-[20px] text-[14px] font-[600] text-[#111827]">{invoice.tenant}</td>
                                <td className="p-[20px] text-[14px] text-[#6B7280]">{invoice.asset}</td>
                                <td className="p-[20px] text-[16px] font-[700] text-[#111827]">${invoice.amount.toLocaleString()}</td>
                                <td className="p-[20px] text-[14px] text-[#6B7280]">{invoice.dueDate}</td>
                                <td className="p-[20px]">
                                    <span className={`px-[10px] py-[4px] rounded-[4px] text-[10px] font-[800] uppercase ${invoice.status === 'Paid' ? 'bg-[#10B981]/10 text-[#10B981]' :
                                ...
                                <td className="p-[20px] text-right">
                                    <button className="text-[13px] font-[700] text-[#1D4ED8] hover:underline">View Details</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-[40px]">
                <Card className="bg-[#111827] border-none p-[48px] text-white shadow-xl">
                    <h3 className="text-[20px] font-[700] mb-[24px]">Automated Billing</h3>
                    <div className="space-y-[24px]">
                        {[
                            ...
                            <div key={item.t} className="flex justify-between items-center gap-[16px] p-[20px] bg-white/5 rounded-[12px] border border-white/10">
                                <div className="space-y-[4px]">
                                    <p className="text-[15px] font-[700]">{item.t}</p>
                                    <p className="text-[13px] text-white/50">{item.d}</p>
                                </div>
                                <div className={`w-[40px] h-[20px] rounded-full relative transition-colors ${item.active ? 'bg-[#1D4ED8]' : 'bg-white/10'}`}>
                ...
                <Card className="bg-[#FFFFFF] border-[#D1D5DB]/30 p-[48px] flex flex-col justify-center text-center space-y-[24px]">
                    ...
                    <div>
                        <h3 className="text-[22px] font-[700] text-[#111827]">98.2% Collection Rate</h3>
                    ...
                    <button className="px-[24px] py-[14px] bg-[#111827] text-white font-[700] rounded-[8px] hover:bg-[#1D4ED8] transition-all text-[14px]">
