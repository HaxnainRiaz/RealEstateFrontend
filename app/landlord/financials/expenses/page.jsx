...
export default function LandlordExpensesPage() {
    ...
    return (
        <div className="space-y-[40px]">
            <div>
                <span className="text-[12px] font-[700] text-[#1D4ED8] uppercase tracking-[0.1em] mb-[8px] block">Cost Analysis</span>
                <h1 className="text-[32px] font-[700] text-[#111827]">Expense Management</h1>
                <p className="text-[16px] text-[#6B7280]">Comprehensive tracking of operational costs and budget adherence.</p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-[40px]">
                <div className="lg:col-span-8 space-y-[32px]">
                    <Card className="bg-[#FFFFFF] border-[#D1D5DB]/30 p-[48px]">
                        <div className="flex justify-between items-center mb-[32px]">
                            <h3 className="text-[20px] font-[700] text-[#111827]">Expense Categories</h3>
                            <button className="px-[16px] py-[8px] bg-[#F9FAFB] border border-[#D1D5DB] rounded-[6px] text-[12px] font-[700] text-[#6B7280]">
                                Export Report
                            </button>
                        </div>
                            ...
                            {EXPENSES.map((expense, i) => {
                                ...
                                return (
                                    <motion.div
                                        ...
                                        className="p-[24px] bg-[#F9FAFB] rounded-[16px] border border-[#D1D5DB]/20 hover:border-[#1D4ED8] transition-all"
                                    >
                                        <div className="flex justify-between items-start mb-[16px]">
                                            <div>
                                                <h4 className="text-[17px] font-[700] text-[#111827] mb-[4px]">{expense.category}</h4>
                                                <p className="text-[12px] text-[#6B7280]">
                                                    Budget: ${expense.budget.toLocaleString()} • Used: {budgetUsed.toFixed(1)}%
                                                </p>
                                            </div>
                                            <span className={`text-[14px] font-[700] ${expense.trend.startsWith('-') ? 'text-[#10B981]' : expense.trend === '0%' ? 'text-[#6B7280]' : 'text-[#EF4444]'}`}>
                                                {expense.trend}
                                            </span>
                                        </div>
                                        <div className="grid grid-cols-2 gap-[24px] mb-[16px]">
                                            <div>
                                                <p className="text-[11px] text-[#6B7280] uppercase font-[700] mb-[4px]">Monthly Avg</p>
                                                <p className="text-[20px] font-[700] text-[#111827]">${expense.monthly.toLocaleString()}</p>
                                            </div>
                                            <div>
                                                <p className="text-[11px] text-[#6B7280] uppercase font-[700] mb-[4px]">Year to Date</p>
                                                <p className="text-[20px] font-[700] text-[#EF4444]">${expense.ytd.toLocaleString()}</p>
                                            </div>
                                        </div>
                    ...
                    <Card className="bg-[#FFFFFF] border-[#D1D5DB]/30 p-[48px]">
                        <h3 className="text-[20px] font-[700] text-[#111827] mb-[32px]">Expense Trend Analysis</h3>
                        <div className="aspect-[21/9] bg-[#F9FAFB] border border-dashed border-[#D1D5DB] rounded-[16px] flex items-center justify-center">
                            <span className="text-[14px] font-[600] text-[#D1D5DB]">DYNAMIC EXPENSE CHART</span>
                        </div>
                    </Card>
                </div>
                ...
                <div className="lg:col-span-4 space-y-[32px]">
                    <Card className="bg-[#111827] border-none p-[40px] text-white shadow-xl">
                        <h3 className="text-[18px] font-[700] mb-[24px]">Cost Reduction Opportunities</h3>
                    ...
                    <Card className="bg-[#FFFFFF] border-[#D1D5DB]/30 p-[40px]">
                        <h4 className="text-[16px] font-[700] text-[#111827] mb-[20px]">Expense Distribution</h4>
                        <div className="space-y-[20px]">
                                ...
                                return (
                                    <div key={exp.id} className="space-y-[8px]">
                                        <div className="flex justify-between text-[12px] font-[700]">
                                            <span className="text-[#6B7280] uppercase tracking-[0.05em]">{exp.category}</span>
                            ...
                            <div className="pt-[12px] border-t border-[#D1D5DB]/20">
                                <p className="text-[11px] text-[#6B7280]">Other categories: {((EXPENSES.slice(3).reduce((sum, exp) => sum + exp.monthly, 0) / totalMonthly) * 100).toFixed(1)}%</p>
                            </div>
                        </div>
                    </Card>
                </div>
